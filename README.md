# Excel Data Analysis Portfolio
Welcome to my portfolio! This repository documents my journey through the Excel Specialization, showcasing my ability to handle increasingly complex datasets.

## 🎓 Certifications
* **Excel Skills for Business: Essentials** - [https://coursera.org/share/244c490b423b6973f5c205e82c594a60]
* **Excel Skills for Business: Intermediate I** - [https://coursera.org/share/fa687c48ad460d3f9925a41ec3a9988d]
* **Excel Skills for Business: Intermediate II** - *In Progress*

---

## 📂 Featured Projects

### 1. Sales Data Cleaning & Reporting (Intermediate I)
* **Goal:** Transform 5,000+ rows of messy sales data into a functional dashboard.
* **Key Skills:** PivotTables, VLOOKUP, Named Ranges, Data Validation.
* **The Result:** Reduced reporting time by 40% using automated lookup tables.
* [View Folder](./02_Intermediate_Level_1/)
